...
int zvfs_poll_internal(struct zvfs_pollfd *fds, int nfds, k_timeout_t timeout)
{
	...
	for (pfd = fds, i = nfds; i--; pfd++) {
		...
		ctx = z_get_fd_obj_and_vtable(pfd->fd, &vtable, &lock);
		if (ctx == NULL) {
		...
		result = z_fdtable_call_ioctl(vtable, ctx, ZFD_IOCTL_POLL_PREPARE, pfd, &pev,
					      pev_end);
		if (result == -EALREADY) {
	...
	if (offload) {
		...
		return z_fdtable_call_ioctl(offl_vtable, offl_ctx, ZFD_IOCTL_POLL_OFFLOAD, fds,
					    nfds, poll_timeout);
	}
	...
	do {
		...
		for (pfd = fds, i = nfds; i--; pfd++) {
			...
			result =
				z_fdtable_call_ioctl(vtable, ctx, ZFD_IOCTL_POLL_UPDATE, pfd, &pev);
			k_mutex_unlock(lock);
