...
static void modbus_serial_tx_on(struct modbus_context *ctx)
{
	...
	if (IS_ENABLED(CONFIG_MODBUS_SERIAL_ASYNC_API)) {
		int err = uart_tx(cfg->dev, cfg->uart_buf, cfg->uart_buf_ctr, SYS_FOREVER_US);
		if (err) {
			LOG_ERR("uart_tx returns error = %d", err);
		}
	} else {
...
static void modbus_serial_rx_on(struct modbus_context *ctx)
{
	...
	if (IS_ENABLED(CONFIG_MODBUS_SERIAL_ASYNC_API)) {
		/* Reset memory before receiving */
	    memset(cfg->uart_buf, 0, sizeof(cfg->uart_buf));

		int err = uart_rx_enable(cfg->dev, cfg->uart_buf, sizeof(cfg->uart_buf), cfg->rtu_timeout);
		if (err) {
			LOG_ERR("uart_rx_enable returns error = %d", err);
		}
	} else {
		       ...
		       struct modbus_iface_param param)
{
	...
	if (IS_ENABLED(CONFIG_MODBUS_SERIAL_ASYNC_API)) {
		int err = uart_callback_set(cfg->dev, uart_cb_async_handler, ctx);
		if (err != 0) {
			LOG_ERR("uart_callback_set returns error = %d", err);
		}
	} else {
