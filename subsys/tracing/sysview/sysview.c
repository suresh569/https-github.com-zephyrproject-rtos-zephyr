...
#define USER_EVENT_MAXSTR 20 /* Maximum string length supported by user event */
...
void sys_trace_user_event(const char *name, uint32_t arg0, uint32_t arg1)
{
	...
	uint8_t a_packet[SEGGER_SYSVIEW_INFO_SIZE + 2 *
		SEGGER_SYSVIEW_QUANTA_U32 + USER_EVENT_MAXSTR + 1];
	uint8_t *payload;

	payload = SEGGER_SYSVIEW_PREPARE_PACKET(a_packet);
	payload = SEGGER_SYSVIEW_EncodeString(payload, name, USER_EVENT_MAXSTR);
	payload = SEGGER_SYSVIEW_EncodeU32(payload, arg0);
	payload = SEGGER_SYSVIEW_EncodeU32(payload, arg1);
	SEGGER_SYSVIEW_SendPacket(a_packet, payload, TID_USER_EVENT);
}
