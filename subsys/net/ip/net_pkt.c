...
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	buf = pkt_alloc_buffer(pool, alloc_len, reserve,
			       timeout, caller, line);
#else
	buf = pkt_alloc_buffer(pool, alloc_len, reserve, timeout);
#endif
...
#endif
{
	...
	if (net_pkt_iface(pkt) && net_if_l2(net_pkt_iface(pkt))->alloc != NULL) {
		ret = net_if_l2(net_pkt_iface(pkt))->alloc(net_pkt_iface(pkt),
							   pkt, size, proto,
							   timeout);
		if (ret != -ENOTSUP) {
			return ret;
