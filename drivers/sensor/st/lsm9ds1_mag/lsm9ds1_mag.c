 ...
 * Warning : the real values of the sampling frequencie are often not an integer.
...
static int lsm9ds1_mag_freq_to_odr_val(uint16_t freq)
{
	...
	for (i = 0; i < ARRAY_SIZE(lsm9ds1_mag_odr_map); i++) {
...
static int lsm9ds1_mag_freq_to_fast_odr_val(uint16_t freq)
{
	...
	for (i = ARRAY_SIZE(lsm9ds1_mag_fast_odr_map); i >= 0; i--) {
...
static const struct sensor_driver_api lsm9ds1_mag_api_funcs = {
