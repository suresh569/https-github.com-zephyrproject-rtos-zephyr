...
static int i2s_esp32_reload_dma(const struct device *dev, enum i2s_dir i2s_dir)
{
	const struct i2s_esp32_cfg *dev_cfg = dev->config;
	struct i2s_esp32_data *const dev_data = dev->data;
	...
	int err;
	uint32_t block_size = stream->i2s_cfg.block_size;
...
static int i2s_esp32_start_dma(const struct device *dev, enum i2s_dir i2s_dir)
{
	...
	dma_blk.block_size = stream->i2s_cfg.block_size;
	if (i2s_dir == I2S_DIR_RX) {
	...
	dma_cfg.dma_slot = ESP_GDMA_TRIG_PERIPH_I2S0;/*<--*/
...
static int i2s_esp32_tx_stream_start(const struct device *dev)
{
	...
	err = i2s_esp32_queue_get(&stream->mem_block_queue, &stream->mem_block, &mem_block_size);
	if (err < 0) {
		return -ENOMEM;
	}
	k_sem_give(&stream->sem);
				      ...
				      int status)
{
	...
	err = i2s_esp32_reload_dma(dev, I2S_DIR_RX);
	if (err < 0) {
		goto rx_disable;
	}

	err = i2s_esp32_queue_put(&stream->mem_block_queue, mem_block_tmp,
				      ...
				      int status)
{
	...
	if (status < 0) {
		LOG_DBG("TX status bad (%i)", status);
		stream->state = I2S_STATE_ERROR;
	}
	...
	err = i2s_esp32_reload_dma(dev, I2S_DIR_TX);
	if (err < 0) {
		goto tx_disable;
	}
...
static int i2s_esp32_initialize(const struct device *dev)
{
	...
	err = k_sem_init(&dev_data->rx.sem, 0, I2S_ESP32_RX_BLOCK_COUNT);
	if (err != 0) {
		return -EINVAL;
	}

	err = k_sem_init(&dev_data->tx.sem, I2S_ESP32_TX_BLOCK_COUNT,
...
static int i2s_esp32_read(const struct device *dev, void **mem_block, size_t *size)
{
	...
	ret = i2s_esp32_queue_get(&dev_data->rx.mem_block_queue, mem_block, size);
	if (ret < 0) {
		return -EIO;
	}
...
static int i2s_esp32_write(const struct device *dev, void *mem_block, size_t size)
{
	...
	ret = k_sem_take(&dev_data->tx.sem,
			 SYS_TIMEOUT_MS(dev_data->tx.i2s_cfg.timeout));
	if (ret < 0) {
		return ret;
	}

	i2s_esp32_queue_put(&dev_data->tx.mem_block_queue, mem_block, size);
...
#define I2S_ESP32_INIT(index)                                                                      \
	...
	static const struct i2s_esp32_cfg i2s_esp32_config_##index = {                             \
		...
		.clock_subsys = (clock_control_subsys_t)DT_CLOCKS_CELL(I2S(index), offset),        \
                                                                                                   ...
                                                                                                   \
	static struct queue_item i2s_esp32_rx_##index##_ring_buf[I2S_ESP32_RX_BLOCK_COUNT + 1];    \
	static struct queue_item i2s_esp32_tx_##index##_ring_buf[I2S_ESP32_TX_BLOCK_COUNT + 1];    \
                                                                                                   \
