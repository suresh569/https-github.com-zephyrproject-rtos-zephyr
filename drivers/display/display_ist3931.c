 ...
 */
#define DT_DRV_COMPAT ist_ist3931
...
typedef bool (*ist3931_bus_ready_fn)(const struct device *dev);
typedef int (*ist3931_write_bus_fn)(const struct device *dev, uint8_t *buf, size_t len,
				    bool command);

struct ist3931_config {
	struct i2c_dt_spec bus;
	ist3931_bus_ready_fn bus_ready;
			     ...
			     uint32_t num_bytes)
{
	...
	ret = i2c_write_dt(&config->bus, i2c_write_buf, num_bytes * 2);
	return ret;
};
...
static inline int ist3931_set_ct(const struct device *dev)
{
	...
	ist3931_write_bus(dev, cmd_buf, 1, 2);
	return 0;
};
...
static inline int ist3931_set_fr(const struct device *dev)
{
	...
	ist3931_write_bus(dev, cmd_buf, 1, 3);
	return 0;
};
...
static inline int ist3931_driver_display_control(const struct device *dev)
{
	...
	ist3931_write_bus(dev, &cmd_buf, 1, 1);
	return 0;
};
...
static inline int ist3931_driver_set_com_pad_map(const struct device *dev)
{
	...
	uint8_t cmd_buf3 = IST3931_CMD_EXIT_ENTRY;

	ist3931_write_bus(dev, &cmd_buf[0], 1, 5);
	k_sleep(K_MSEC(IST3931_CMD_DELAY));
	ist3931_write_bus(dev, &cmd_buf3, 1, 1);
	return 0;
};
...
static inline int ist3931_driver_set_ay(const struct device *dev, uint16_t y)
{
	...
	ist3931_write_bus(dev, &cmd_buf[0], 1, 2);
	return 0;
};
...
static int ist3931_blanking_on(const struct device *dev)
{

	return ist3931_driver_sleep_on_off(dev, false);
}

static int ist3931_blanking_off(const struct device *dev)
{
