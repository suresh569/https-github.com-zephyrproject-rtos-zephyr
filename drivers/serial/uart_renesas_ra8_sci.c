...
static int uart_ra_sci_poll_in(const struct device *dev, unsigned char *c)
{
	...
	R_SCI0_Type *reg = data->sci.p_reg;

	/* Check if async reception was enabled */
	if (IS_ENABLED(CONFIG_UART_ASYNC_API) && reg->SCR_b.RIE) {
		return -EBUSY;
	}

	if (IS_ENABLED(CONFIG_UART_RA_SCI_UART_FIFO_ENABLE) ? reg->SSR_FIFO_b.RDF == 0U
							    : reg->SSR_b.RDRF == 0U) {
...
static int uart_ra_sci_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	...
	if (IS_ENABLED(CONFIG_UART_RA_SCI_UART_FIFO_ENABLE) && data->sci.fifo_depth > 0) {
		while ((size - num_tx > 0) && reg->FDR_b.T != 0x10U) {
			/* FTSR flag will be cleared with byte write to TDR register */

			/* Send a character (8bit , parity none) */
			reg->TDR = tx_data[num_tx++];
		}
	} else {
		if (size > 0 && reg->SSR_b.TDRE) {
...
static int uart_ra_sci_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	...
	if (IS_ENABLED(CONFIG_UART_RA_SCI_UART_FIFO_ENABLE) && data->sci.fifo_depth > 0) {
		while ((size - num_rx > 0) && reg->FDR_b.R > 0U) {
			/* FRSR.DR flag will be cleared with byte write to RDR register */

			/* Receive a character (8bit , parity none) */
			rx_data[num_rx++] = reg->RDR;
		}
		if (reg->SSR_FIFO_b.DR == 0U) {
			reg->SSR_FIFO_b.RDF = 1U;
			reg->SSR_FIFO_b.DR = 1U;
		}
	} else {
		if (size > 0 && reg->SSR_b.RDRF) {
			/* Receive a character (8bit , parity none) */
			rx_data[num_rx++] = reg->RDR;
		}
	}
...
static void uart_ra_sci_irq_tx_enable(const struct device *dev)
{
	...
	reg->SCR |= (BIT(R_SCI0_SCR_TIE_Pos) | BIT(R_SCI0_SCR_TEIE_Pos));
}
...
static int uart_ra_sci_irq_tx_ready(const struct device *dev)
{
	...
	return (reg->SCR_b.TIE == 1U) &&
	       (data->csr & (BIT(R_SCI0_SSR_TDRE_Pos) | BIT(R_SCI0_SSR_TEND_Pos)));
}
...
static int uart_ra_sci_irq_tx_complete(const struct device *dev)
{
	...
	return (reg->SCR_b.TEIE == 1U) && (data->csr & BIT(R_SCI0_SSR_TEND_Pos));
}
...
static int uart_ra_sci_irq_rx_ready(const struct device *dev)
{
	...
	return (reg->SCR_b.RIE == 1U) &&
	       ((data->csr & BIT(R_SCI0_SSR_RDRF_Pos)) ||
		(IS_ENABLED(CONFIG_UART_RA_SCI_UART_FIFO_ENABLE) && reg->SSR_FIFO_b.DR == 1U));
}
...
static int uart_ra_sci_irq_is_pending(const struct device *dev)
{
	...
	const uint32_t ccr0 = reg->SCR;
	const uint32_t csr = reg->SSR;

	const bool tx_pending = ((ccr0 & BIT(R_SCI0_SCR_TIE_Pos)) &&
				 (csr & (BIT(R_SCI0_SSR_TEND_Pos) | BIT(R_SCI0_SSR_TDRE_Pos))));
	const bool rx_pending =
		((ccr0 & BIT(R_SCI0_SCR_RIE_Pos)) &&
		 ((csr & (BIT(R_SCI0_SSR_RDRF_Pos) | BIT(R_SCI0_SSR_PER_Pos) |
...
static int uart_ra_sci_irq_update(const struct device *dev)
{
	...
	if (data->csr & BIT(R_SCI0_SSR_PER_Pos)) {
		cfclr |= BIT(R_SCI0_SSR_PER_Pos);
	}
	if (data->csr & BIT(R_SCI0_SSR_FER_Pos)) {
		cfclr |= BIT(R_SCI0_SSR_FER_Pos);
	}
	if (data->csr & BIT(R_SCI0_SSR_ORER_Pos)) {
		cfclr |= BIT(R_SCI0_SSR_ORER_Pos);
	}
...
static inline void async_replace_rx_buffer(const struct device *dev)
{
	...
	if (data->rx_next_buffer != NULL) {
		...
		R_SCI_UART_Read(&data->sci, data->rx_buffer, data->rx_buffer_cap);
...
static inline void uart_ra_sci_async_timer_start(struct k_work_delayable *work, size_t timeout)
{
				...
				int32_t timeout)
{
	...
	uart_ra_sci_async_timer_start(&data->tx_timeout_work, timeout);
...
static inline void disable_tx(const struct device *dev)
{
	...
	data->sci.p_reg->SCR &= (uint32_t) ~(R_SCI0_SCR_TIE_Msk | R_SCI0_SCR_TEIE_Msk);

	/*
	 * Make sure no transmission is in progress. Setting SCR_b.TE to 0 when CSR_b.TEND
	 * is 0 causes SCI peripheral to work abnormally.
	 */
	while (data->sci.p_reg->CSR_b.TEND != 1U) {
	}

	data->sci.p_reg->SCR &= (uint32_t) ~(R_SCI0_SCR_TE_Msk);
...
static int uart_ra_sci_async_tx_abort(const struct device *dev)
{
	...
	R_SCI_UART_Abort(&data->sci, UART_DIR_TX);
...
static void uart_ra_sci_async_tx_timeout(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct uart_ra_sci_data *data =
		CONTAINER_OF(dwork, struct uart_ra_sci_data, tx_timeout_work);
...
static int uart_ra_sci_async_rx_disable(const struct device *dev)
{
	...
	err = fsp_err_to_errno(R_SCI_UART_ReadStop(&data->sci, &remaining_byte));
	if (err != 0) {
		goto unlock;
	}
...
static void uart_ra_sci_async_rx_timeout(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct uart_ra_sci_data *data =
...
static int uart_ra_sci_init(const struct device *dev)
{
	...
	k_work_init_delayable(&data->tx_timeout_work, uart_ra_sci_async_tx_timeout);
	k_work_init_delayable(&data->rx_timeout_work, uart_ra_sci_async_rx_timeout);
...
#if defined(CONFIG_UART_ASYNC_API)
	uart_ra_sci_async_timer_start(&data->rx_timeout_work, data->rx_timeout);

	if (data->fsp_config.p_transfer_rx) {
		/*
		 * The RX DTC is set to TRANSFER_IRQ_EACH, triggering an interrupt for each received
		 * byte. However, the sci_uart_rxi_isr function currently only handles the
...
#define UART_RA_SCI_IRQ_CONFIG_INIT(index)                                                         \
	do {                                                                                       \
		R_ICU->IELSR[DT_IRQ_BY_NAME(DT_INST_PARENT(index), rxi, irq)] =                    \
			ELC_EVENT_SCI_RXI(DT_PROP(DT_INST_PARENT(index), channel));                \
		R_ICU->IELSR[DT_IRQ_BY_NAME(DT_INST_PARENT(index), txi, irq)] =                    \
			ELC_EVENT_SCI_TXI(DT_PROP(DT_INST_PARENT(index), channel));                \
		R_ICU->IELSR[DT_IRQ_BY_NAME(DT_INST_PARENT(index), tei, irq)] =                    \
			ELC_EVENT_SCI_TEI(DT_PROP(DT_INST_PARENT(index), channel));                \
		R_ICU->IELSR[DT_IRQ_BY_NAME(DT_INST_PARENT(index), eri, irq)] =                    \
			ELC_EVENT_SCI_ERI(DT_PROP(DT_INST_PARENT(index), channel));                \
                                                                                                   \
...
#define UART_RA_SCI_INIT(index)                                                                    \
	...
	{                                                                                          \
		...
		return 0;                                                                          \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(index, uart_ra_sci_init_##index, PM_DEVICE_DT_INST_GET(index),       \
