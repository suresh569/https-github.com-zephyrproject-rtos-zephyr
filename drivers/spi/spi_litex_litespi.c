...
static int spi_litex_set_frequency(const struct device *dev, const struct spi_config *config)
{
	...
	uint32_t divisor = DIV_ROUND_UP(CPU_FREQ, (2 * config->frequency)) - 1;
...
static int spi_litex_xfer(const struct device *dev, const struct spi_config *config)
{
	...
	litex_write8(len, dev_config->core_master_phyconfig_addr +
				  SPIFLASH_CORE_MASTER_PHYCONFIG_LEN_OFFSET);
	litex_write8(width, dev_config->core_master_phyconfig_addr +
				    SPIFLASH_CORE_MASTER_PHYCONFIG_WIDTH_OFFSET);
	litex_write8(mask, dev_config->core_master_phyconfig_addr +
				   SPIFLASH_CORE_MASTER_PHYCONFIG_MASK_OFFSET);
	...
	do {
		...
		if (len != old_len) {
			litex_write8(len * 8, dev_config->core_master_phyconfig_addr +
						      SPIFLASH_CORE_MASTER_PHYCONFIG_LEN_OFFSET);
			old_len = len;
		}
		...
		litex_write(dev_config->core_master_rxtx_addr, len, txd);
		...
		rxd = litex_read(dev_config->core_master_rxtx_addr, len);
...
static int spi_litex_release(const struct device *dev,
			       const struct spi_config *config)
{
