...
struct nxp_video_sdma_data {
	 ...
	 */
	struct smartdma_camera_param params __aligned(4);
				...
				uint32_t channel, int status)
{
	...
	if (data->buf_reload_flag) {
		...
		/* We have finished a frame, dequeue video buffer */
		if (data->frame_idx == 15) {
			/* Last frame in a block, set EOF flag */
			data->active_buf->flags = VIDEO_BUF_EOF;
		} else {
			data->active_buf->flags = VIDEO_BUF_FRAG;
		}
...
static int nxp_video_sdma_stream_start(const struct device *dev)
{
	...
	sdma_config.dma_slot = kSMARTDMA_FlexIO_CameraDiv16Frame;
	...
	data->params.p_buffer_ping_pong = (uint32_t *)data->queued_buf->buffer;
	/* The SDK firmware makes NO mention of this parameter,
	 * But a 4 byte buffer must be provided for this parameter. This
	 * is required because the firmware writes the index of the frame
				  ...
				  struct video_buffer *vbuf)
{
	...
	vbuf->bytesframe = SDMA_VBUF_WIDTH * SDMA_VBUF_HEIGHT * sizeof(uint16_t);
				   ...
				   struct video_caps *caps)
{
	...
	caps->vbuf_per_frame = SDMA_VBUF_HEIGHT / SDMA_LINE_COUNT;
