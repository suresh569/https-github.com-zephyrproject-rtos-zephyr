...
def process_pr(gh, maintainer_file, number):
    ...
    # first. Then process the collaborators by picking one
    # collaborator at a time from each area impacted. Note
    # that the order of the names in each collaborator list
    ...
    # the list... sort of a "promotion ladder". Also, when an
	# area's collaboration list is exhausted, it is dropped
	# from the iterator list.
    #
	# Exammple:
    ...
    iterators = {area.name: iter(maintainer_file.areas[area.name].collaborators) for area in area_counter}
    while iterators:
        for key in list(iterators):
            try:
                if iterators[key] not in collab:
                    collab.append(next(iterators[key]))
                else:
                    next(iterators[key])
