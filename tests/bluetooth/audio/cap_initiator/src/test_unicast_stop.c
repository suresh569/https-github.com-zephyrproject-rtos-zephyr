	...
	struct cap_initiator_test_unicast_stop_fixture *fixture)
{
	...
	for (size_t i = 0U; i < ARRAY_SIZE(fixture->cap_streams); i++) {
		...
		/* Default to the streaming state for the stop tests */
		test_unicast_set_state(cap_stream, &fixture->conns[i % ARRAY_SIZE(fixture->conns)],
...
static ZTEST_F(cap_initiator_test_unicast_stop, test_initiator_unicast_stop_disable)
{
...
static ZTEST_F(cap_initiator_test_unicast_stop, test_initiator_unicast_stop_release)
{
	struct bt_cap_stream *streams[CONFIG_BT_BAP_UNICAST_CLIENT_GROUP_STREAM_COUNT] = {0};
	const struct bt_cap_unicast_audio_stop_param param = {
		.type = BT_CAP_SET_TYPE_AD_HOC,
...
static ZTEST_F(cap_initiator_test_unicast_stop, test_initiator_unicast_stop_state_codec_configured)
{
	struct bt_cap_stream *streams[CONFIG_BT_BAP_UNICAST_CLIENT_GROUP_STREAM_COUNT] = {0};
	const struct bt_cap_unicast_audio_stop_param param = {
		.type = BT_CAP_SET_TYPE_AD_HOC,
	...
	for (size_t i = 0U; i < ARRAY_SIZE(streams); i++) {
		...
		test_unicast_set_state(streams[i], streams[i]->bap_stream.conn,
				       streams[i]->bap_stream.ep, &fixture->preset,
...
static ZTEST_F(cap_initiator_test_unicast_stop, test_initiator_unicast_stop_state_qos_configured)
{
	struct bt_cap_stream *streams[CONFIG_BT_BAP_UNICAST_CLIENT_GROUP_STREAM_COUNT] = {0};
	const struct bt_cap_unicast_audio_stop_param param = {
		.type = BT_CAP_SET_TYPE_AD_HOC,
...
static ZTEST_F(cap_initiator_test_unicast_stop, test_initiator_unicast_stop_state_enabling)
{
	struct bt_cap_stream *streams[CONFIG_BT_BAP_UNICAST_CLIENT_GROUP_STREAM_COUNT] = {0};
	const struct bt_cap_unicast_audio_stop_param param = {
		.type = BT_CAP_SET_TYPE_AD_HOC,
