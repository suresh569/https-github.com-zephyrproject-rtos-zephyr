...
{
	/* Can't do `FILE *file = stdout`, as it will fail in PICOLIBC */
	...
	/* Try again with flockfile(), but don't unlock the file */
	k_thread_create(&test_thread, test_stack, K_THREAD_STACK_SIZEOF(test_stack),
			flockfile_thread, file, LOCK_SHOULD_PASS, NO_UNLOCK_FILE, priority, 0,
			K_NO_WAIT);
	zassert_ok(k_thread_join(&test_thread, K_MSEC(100)));

	/* file is locked in test_thread, the following call will fail */
	zassert_not_ok(ftrylockfile(file));
}
