...
bool add_pa_sync_base_subgroup_cb(const struct bt_bap_base_subgroup *subgroup, void *user_data)
{
	struct bt_cap_commander_broadcast_reception_start_member_param *param = user_data;
	...
	if (ret > ARRAY_SIZE(subgroup_param->metadata)) {
		printk("Cannot fit %d octets into subgroup param with size %zu\n", ret,
	...
	ret = bt_bap_base_subgroup_foreach_bis(subgroup, add_pa_sync_base_subgroup_bis_cb,
					       subgroup_param);
	if (ret < 0) {
		printk("Could not get bis information for %p\n", subgroup_param);
...
static void test_broadcast_reception_start(size_t accepter_count)
{
	...
	reception_start_param.count = accepter_count;
...
static void test_broadcast_reception_stop(size_t accepter_count)
{
...
{
	size_t accepter_count;

	accepter_count = get_dev_cnt() - 2;

	init(accepter_count);

	for (size_t i = 0U; i < accepter_count; i++) {
		scan_and_connect();
	...
	/* TODO: We should use CSIP to find set members */
	discover_cas(accepter_count);
	discover_bass(accepter_count);

	pa_sync_to_broadcaster();

	test_broadcast_reception_start(accepter_count);
	...
	test_broadcast_reception_stop(accepter_count);

	backchannel_sync_wait_any(); /* wait for the acceptor to stop reception */

	/* Disconnect all CAP acceptors */
	disconnect_acl(accepter_count);
