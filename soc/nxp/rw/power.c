...
{
	uint8_t level = ~(DT_PROP(DT_NODELABEL(pin1), wakeup_level)) & 0x1;
...
#if DT_NODE_HAS_STATUS(DT_NODELABEL(pin1), okay)
	gpio_pin_configure(gpio, 25, GPIO_INPUT);
	POWER_ConfigWakeupPin(kPOWER_WakeupPin1, DT_PROP(DT_NODELABEL(pin1), wakeup_level));
