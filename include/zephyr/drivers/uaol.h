...
typedef int (*uaol_api_lock)(const struct device *dev);

typedef int (*uaol_api_unlock)(const struct device *dev);

typedef int (*uaol_api_acquire)(const struct device *dev);

typedef int (*uaol_api_release)(const struct device *dev);
...
__subsystem struct uaol_driver_api {
	uaol_api_lock lock;
	uaol_api_unlock unlock;
	uaol_api_acquire acquire;
	uaol_api_release release;
...
/**
 * @brief Acquire USB Audio Offload Link.
 *
 * @param dev UAOL device instance.
 *
 * @return 0 on success, all other values should be treated as error.
 */
static inline int uaol_acquire(const struct device *dev)
{
...
/**
 * @brief Release USB Audio Offload Link.
 *
 * @param dev UAOL device instance.
 *
 * @return 0 on success, all other values should be treated as error.
 */
static inline int uaol_release(const struct device *dev)
{
	const struct uaol_driver_api *api = dev->api;
	int ret;
					...
					struct uaol_ep_table_entry entry, bool valid)
{
	...
	api->lock(dev);
	ret = api->program_ep_table(dev, stream, entry, valid);
	api->unlock(dev);
...
static inline int uaol_get_capabilities(const struct device *dev, struct uaol_capabilities *caps)
{
	...
	api->lock(dev);
	ret = api->get_capabilities(dev, caps);
	api->unlock(dev);
